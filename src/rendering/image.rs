...
use crate::{device::device::{self, Device}, rendering::memory::find_memory_type};

pub fn create_image(device: &Device, extent: vk::Extent3D, format : vk::Format) -> Result<(vk::Image, vk::DeviceMemory)>{
...
pub fn create_image_view(device : &Device, image: vk::Image, format : vk::Format) -> Result<vk::ImageView>{
    ...
    let img_view = unsafe { device.logical_device.create_image_view(
        &info, 
        None) }?;
    Ok(
        img_view
...
pub fn transition_image_layout(
    device: &Device,
    image : vk::Image,
    format : vk::Format, // to do use later
    ...
    unsafe { device.logical_device.cmd_pipeline_barrier(
        command_buffer, 
        src_stage_mask, 
        dst_stage_mask, 
...
pub fn copy_buffer_to_image(
    ...
    unsafe { device.logical_device.cmd_copy_buffer_to_image(
        command_buffer, 
        src_buffer, 
        dst_img,
...
pub fn create_texture_sampler(device: &Device) -> Result<vk::Sampler>{
    ...
    let sampler = unsafe { device.logical_device.create_sampler(&create_info, None) }?;
    Ok(sampler)
}