use crate::{
    config::{APPLICATION_NAME, ENGINE_NAME, ENGINE_VERSION, VALIDATION_ENABLED},
    utils,
};
use std::{
    collections::HashSet,
...
const PORTABILITY_ENUMERATION_EXTENSION_NAME: &std::ffi::CStr =
    ash::khr::portability_enumeration::NAME;
const GET_PHYSICAL_DEVICE_PROPERTIES2_EXTENSION_NAME: &std::ffi::CStr =
...
fn create_instance(window: &Window, entry: &Entry) -> Result<Instance> {
    ...
    if VALIDATION_ENABLED {
        if !available_layers
            .iter()
            .any(|l| utils::vk_to_cstr(l) == VALIDATION_LAYER)
        {
