...
use crate::rendering::{cleanup::DeletionQueue, vertex::Vertex, vulkan_context::{create_indices_buffer, create_vertex_buffer}};
...
impl Mesh{
    ...
    pub fn new_cube( instance: &ash::Instance, device: &ash::Device, physical_device: vk::PhysicalDevice, deletion_queue :&mut DeletionQueue) -> Result<Self>{
        ...
        let vertices = [
            // Front face
            Vertex::new([-0.5, -0.5,  0.5], [1.0, 0.0, 0.0]), // 0
            Vertex::new([ 0.5, -0.5,  0.5], [0.0, 1.0, 0.0]), // 1
            Vertex::new([ 0.5,  0.5,  0.5], [0.0, 0.0, 1.0]), // 2
            Vertex::new([-0.5,  0.5,  0.5], [1.0, 1.0, 1.0]), // 3
            // Back face
            Vertex::new([-0.5, -0.5, -0.5], [1.0, 0.0, 1.0]), // 4
            Vertex::new([ 0.5, -0.5, -0.5], [0.0, 1.0, 1.0]), // 5
            Vertex::new([ 0.5,  0.5, -0.5], [1.0, 1.0, 0.0]), // 6
            Vertex::new([-0.5,  0.5, -0.5], [0.0, 0.0, 0.0]), // 7
        ];
        let indices: [u32; 36] = [
            0, 1, 2, 2, 3, 0, // Front
            1, 5, 6, 6, 2, 1, // Right
